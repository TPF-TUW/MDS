...
namespace F02
{
    public partial class F02 : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void NewData()
        {
            ...
            txeID.Text = new DBQuery("SELECT CASE WHEN ISNULL(MAX(OIDBranch), '') = '' THEN 1 ELSE MAX(OIDBranch) + 1 END AS NewNo FROM " + this.dbBranch).getString();
        ...
        private void gvBranch_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                DataTable dtCP = (DataTable)gcBranch.DataSource;
                if (dtCP.Rows.Count > 0)
                {
                    ...
                    txeID.Text = drCP["OID Branch"].ToString();
                    glueCompany.EditValue = drCP["OIDCOMPANY"].ToString();
                    txeBranchNo.Text = drCP["Branch No."].ToString();
                    txeBranchName.Text = drCP["Branch Name"].ToString();
                    glueBranchType.EditValue = drCP["Branch Type"].ToString();
 
                    txeCREATE.Text = drCP["Created By"].ToString();
                    txeCDATE.Text = drCP["Created Date"].ToString();
                }
            }
        }
