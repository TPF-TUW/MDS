...
namespace F05
{
    public partial class F05 : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void LoadData()
        {
            ...
            new ObjDevEx.setGridControl(gcUser, gvUser, sbSQL).getData(false, false, false, true);
        ...
        private void LoadFunction(string UserID = "")
        {
            ...
            if (lblStatus.Text == "* Edit User")
            {
                ...
                sbSQL.Append("GROUP BY FL.GroupFunction, GF.[Group Function], FL.FunctionNo, FL.FunctionName, FA.AllowDenyStatus, FA.ReadWriteStatus ");
                sbSQL.Append("ORDER BY GF.[Group Function], FL.FunctionNo ");
            }
            else
            {
            ...
            new ObjDevEx.setGridControl(gcFunction, gvFunction, sbSQL).getData(false, false, false, true);

            gvFunction.Columns[0].Group();
            gvFunction.Columns["GroupFunction"].Visible = false;
            gvFunction.Columns["Version"].Visible = false;

            dtFuncName = new DBQuery(sbSQL).getDataTable();
        }
        ...
        private void NewData()
        {
            ...
            txeID.Text = new DBQuery("SELECT CASE WHEN ISNULL(MAX(OIDUSER), '') = '' THEN 1 ELSE MAX(OIDUSER) + 1 END AS NewNo FROM Users").getString();
            txeUserName.Text = "";
            txeFullName.Text = "";
            txtPassword.Text = "";
                        ...
                        if (sbSQL.Length > 0)
                        {
                            try
                            {
                                ...
                                if (chkSAVE == true)
                                {
                                    ...
                                    if (lblStatus.Text == "* Add User")
                                    {
                                        string UID = new DBQuery("SELECT OIDUSER FROM Users WHERE (UserName=N'" + txeUserName.Text.Trim().Replace("'", "''") + "') ").getString();
                                    ...
                                    else if (lblStatus.Text == "* Edit User")
                                    {
                                        ...
                                        if (dtFUNC.Rows.Count > 0)
                                        {
                                            foreach (DataRow row in dtFUNC.Rows)
                                            {
                                                ...
                                                sbSQL.Append("    ReadWriteStatus='" + row["ReadWrite"].ToString() + "', AllowDenyStatus='" + row["Permission"].ToString() + "' ");
        ...
        private void glueCompany_EditValueChanged(object sender, EventArgs e)
        {
            if (glueCompany.Text.Trim() != "")
            {
                ...
                new ObjDevEx.setGridLookUpEdit(glueBranch, sbSQL, "Branch Name", "ID").getData();
        ...
        private void glueBranch_EditValueChanged(object sender, EventArgs e)
        {
            ...
            new ObjDevEx.setGridLookUpEdit(glueDepartment, sbSQL, "Department", "ID").getData();
            glueDepartment.Focus();
        }
