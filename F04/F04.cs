...
namespace F04
{
    public partial class F04 : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void LoadData()
        {
            ...
            new ObjDevEx.setGridControl(gcFolder, gvFolder, sbSQL).getData(false, false, false, true);
            ...
            new ObjDevEx.setGridLookUpEdit(glueGroup, sbGROUP, "Group Function", "ID").getData();
        ...
        private void NewData()
        {
            ...
            txeID.Text = new DBQuery("SELECT CASE WHEN ISNULL(MAX(OIDFunction), '') = '' THEN 1 ELSE MAX(OIDFunction) + 1 END AS NewNo FROM FunctionList").getString();
            txeCode.Text = "";
            txeName.Text = "";
        ...
        private void txeCode_Leave(object sender, EventArgs e)
        {
            if (txeCode.Text.Trim() != "")
            {
                ...
                string[] arrFunc = new DBQuery(sbSQL).getMultipleValue();
                if (arrFunc.Length > 0)
                {
                ...
                else
                {
                    if (txeName.Text.Trim() != "")
                    {
                        ...
                        string strNo = new DBQuery(sbSQL).getString();
        ...
        private void txeVersion_Leave(object sender, EventArgs e)
        {
            if (txeVersion.Text.Trim() != "")
            {
                ...
                string strVER = new DBQuery(sbSQL).getString();
                if (strVER != "")
                {
                    if (lblStatus.Text == "* Add Function")
                    {
